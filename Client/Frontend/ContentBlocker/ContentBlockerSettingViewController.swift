...
class ContentBlockerSettingsTableView: SettingsTableViewController {
    ...
    func moreInfoTapped() {
        ...
        try! viewController.url = NSLocalizedString("settings.trackingprotection.more.url", value: "https://help.qwant.com/help/qwant-mobile/about-tracking-protection/?client=qwantbrowser", comment: "URL for more info about tracking protection").asURL()
...
class ContentBlockerSettingViewController: ContentBlockerSettingsTableView {
    ...
    init(prefs: Prefs) {
        self.prefs = prefs
        currentEnabledState = ContentBlockerHelper.EnabledState(rawValue: prefs.stringForKey(ContentBlockerHelper.PrefKeyEnabledState) ?? "") ?? .on
